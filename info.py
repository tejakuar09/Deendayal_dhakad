...
PICS = (environ.get('PICS', 'https://envs.sh/y9D.jpg https://envs.sh/y9E.jpg https://graph.org/file/425b6f46efc7c6d64105f.jp ')).split()  # Sample pic
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1133549523').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-2321973166').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002036849852'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002396156493'))  # Bin channel id (make sure bot is admin)
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002135122055')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002321973166')  # Request channel id (make sure bot is admin)
...
OWNER_UPI_ID = environ.get('OWNER_UPI_ID', 'https://t.me/Hindi_movie_uplod/32')
...
HOW_TO_VERIFY = environ.get('HOW_TO_VERIFY', 'https://t.me/how_to_downlodm')  # How to open tutorial link for verification
